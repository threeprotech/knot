...
export default async function HomePage() {
  ...
  const [{ data: eventRows, error: eventError }, { data: alertRows, error: alertError }] =
    await Promise.all([
      supabase.from("events").select("*").eq("visibility", "public").order("starts_at"),
      supabase.from("alerts").select("*").eq("visibility", "public").order("starts_at", { ascending: false }),
    ]);

  const events = eventError
    ? []
    : sortEventsByStart(
        ((eventRows as AlumniEvent[]) || []).filter((event) => isUpcomingOrActiveEvent(event)),
      );
  const alerts = alertError
    ? []
  ...
  return (
    <>
      ...
      <PublicHomeFeed alerts={alerts} events={events} />
    </>
  );
}
